...
async def cek_fase_l(db, existing) -> None:
    ...
    # DRIFT: literal lini di frontend (harus dari master)
    literal = []
    for p in files(FE, (".js", ".jsx")):
        if re.search(r'["\'](woven|knit|printing)["\']', read(p)) and "LineFilter" not in p.name:
            literal.append(p.relative_to(FE).as_posix())
    add("L", BELUM if literal else SELESAI, "literal lini di frontend",
        f"{len(literal)} berkas memuat \"woven\"/\"knit\"/\"printing\" — "
        f"sebagian SAH (fabric_type), wajib dipilah saat FASE L: "
        f"{', '.join(literal[:4])}{'…' if len(literal) > 4 else ''}"
        if literal else "tidak ada")
...
async def cek_fase_u(db, existing) -> None:
    kode = sorted([u.get("code", "") for u in await db.uoms.find({}, {"_id": 0, "code": 1}).to_list(100)])
    add("U", SELESAI if {"KG", "PANEL"} <= set(kode) else BELUM, "master satuan KG & PANEL",
        f"uoms = {kode}")
    # DRIFT NYATA: satuan yang dipakai dokumen vs kode master
    dipakai = set()
        ...
        if coll in existing:
            for v in await db[coll].distinct(f):
                if isinstance(v, str) and v:
                    dipakai.add(v)
    norm = {k.lower() for k in kode}
    hilang = sorted(u for u in dipakai if u.lower() not in norm)
    add("U", DRIFT if hilang else SELESAI, "satuan dokumen ⊆ master satuan",
        ...
        if hilang else f"semua satuan terdaftar: {sorted(dipakai)}")
    # cakupan qty_rolls
    sudah, belum = [], []
